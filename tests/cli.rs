...
fn test_init_command_creates_default_notebook() -> TestResult {
    ...
    assert!(
        get_notebook_entries_path(&rjot_dir, "default").exists(),
        "Default notebook directory should be created by init"
    );
...
fn test_notebook_use_cmd_output() -> TestResult {
    ...
    rjot_cmd()
        ...
        .stderr(predicate::str::contains(
            "Notebook 'nonexistent' not found.",
        ));
    Ok(())
}
...
fn test_jot_in_custom_notebook_via_env_var() -> TestResult {
    ...
    rjot_cmd().arg("init").assert().success();
    rjot_cmd()
        .args(["notebook", "new", "envbook"])
        .assert()
        .success();

    rjot_cmd().arg("note in default").assert().success();
    ...
    assert_eq!(fs::read_dir(entries_dir)?.count(), 1);
    let entry_path = fs::read_dir(get_notebook_entries_path(&rjot_dir, "envbook"))?
        .next()
        .unwrap()?
        .path();
    let content = fs::read_to_string(entry_path)?;
...
fn test_info_paths_reflects_notebook_context() -> TestResult {
    ...
    rjot_cmd().arg("init").assert().success();
    rjot_cmd()
        .args(["notebook", "new", "infobook"])
        .assert()
        .success();

    let rjot_dir_path = PathBuf::from(env::var("RJOT_DIR").unwrap());

    let default_notebook_path_str = get_notebook_entries_path(&rjot_dir_path, "default")
        .to_string_lossy()
        .into_owned();
    let infobook_path_str = get_notebook_entries_path(&rjot_dir_path, "infobook")
        .to_string_lossy()
        .into_owned();
    ...
    rjot_cmd()
        ...
        .stdout(predicate::str::contains(format!(
            "Active Notebook Dir: \"{}\"",
            default_notebook_path_str
        )))
        .stdout(predicate::str::contains(
            "Env Var (RJOT_ACTIVE_NOTEBOOK): Not set",
        ));
    ...
    rjot_cmd()
        ...
        .stdout(predicate::str::contains(
            "Env Var (RJOT_ACTIVE_NOTEBOOK): infobook",
        ));
...
fn test_find_command_scoped_to_notebook() -> TestResult {
    ...
    rjot_cmd()
        .args([
            "--notebook-opt",
            "findnb",
            "UniqueToFindNB CommonTextInFindNB",
        ])
        .assert()
        .success();
...
fn test_fallback_to_default_notebook_if_env_var_points_to_nonexistent() -> TestResult {
    ...
    rjot_cmd()
        .arg("Note in actual default for fallback test")
        .assert()
        .success();
    ...
    assert!(default_notes
        .iter()
        .any(|s| s.contains("Jot during fallback warning")));
...
fn test_tag_management_in_default_notebook() -> TestResult {
    ...
    rjot_cmd().arg("note for tags").assert().success();
...
fn test_encryption_lifecycle_with_default_notebook() -> TestResult {
    let (_temp_dir, rjot_dir) = setup_test_environment();
    rjot_cmd().args(["init", "--encrypt"]).assert().success();
    ...
    rjot_cmd().args(["decrypt", "--force"]).assert().success();
...
fn test_direct_helper_then_cmd() -> TestResult {
    ...
    rjot_cmd()
        .args(["notebook", "new", "directtest"])
        .assert()
        .success();
    let notebook_path_check = get_notebook_entries_path(&rjot_dir, "directtest");
    assert!(
        notebook_path_check.exists(),
        "Notebook directory 'directtest' should be created by 'rjot notebook new'"
    );
    ...
    assert_eq!(
        ...
        1,
        "Should be one jot in directtest notebook"
    );
    Ok(())
}
